...
__global__ void matMultRowElement(float *M, float *N, float *P, int height_m, int k, int width_n)
{
    int row = blockIdx.y * blockDim.y + threadIdx.y;

    if (row < height_m)
    {
        for (int out_col = 0; out_col < width_n; out_col++)
        {
            float sum = 0;
            for (int K = 0; K < k; K++)
            {
                // printf("Pos: (%d, %d)\n\t M Unraveled: %d, N Unraveled: %d \n", row, out_col, row * k + K, K * width_n + out_col);
                sum += M[row * k + K] * N[K * width_n + out_col];
            }
            P[row * width_n + out_col] = sum;
        }
    }
}
...
int main()
{
    ...
    matMultRowElement<<<GRID_SIZE, BLOCK_SIZE>>>(M_d, N_d, P_d, height_m, k, width_n);
